...
test('öffentliche Demo ist ohne Login erreichbar', async ({ page }) => {
  ...
  await expect(page.getByLabel('Baustelle')).toContainText('Neubau Müller');
  await expect(page.getByRole('button', { name: 'Arbeitszeit eintragen' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Live stempeln' })).toBeVisible();
});
...
test('Baustelle anlegen, auswählen und Arbeit stempeln', async ({ page }) => {
  ...
  await expect(page.getByText('E2E Baustelle').first()).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Laufende Zeit beenden' }).click();
...
test('Eintrag bearbeiten, löschen und wiederherstellen', async ({ page }) => {
  ...
  await page.locator('.entry').filter({ hasText: 'Fenster eingesetzt' }).getByText('Löschen').click();
